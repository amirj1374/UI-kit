...
import { serializeCustomizerPreferences, type AppDirection, type AppLanguage } from '../../utils/customizerPreferences';
...
<template>
  <v-navigation-drawer
    ...
    temporary
    elevation="10"
    :location="direction === 'rtl' ? 'left' : 'right'"
    :width="width"
            ...
            <v-tabs-window-item value="style">
              <div class="app-customizer-panel__content pa-4">
                <section>
                  ...
                  <div class="theme-toggle-container">
                    <div
                      ...
                      @click="emit('update:themeMode', themeMode === 'light' ? 'dark' : 'light')"
                    >
                      <div class="toggle-slider">
                        <div class="toggle-icon">
                          <IconSun v-if="themeMode === 'light'" size="24" stroke-width="2" /><IconMoon
                            v-else
                            size="24"
                            stroke-width="2"
                            color="white"
                ...
                <section>
                  ...
                  <div class="custom-theme-colors">
                    <button
                      ...
                      :key="color.themeName"
                      :class="['color-option', { selected: activeBaseTheme === color.themeName }]"
                      :style="{ background: `conic-gradient(${color.primary} 50%, ${color.secondary} 50%)` }"
                      :aria-label="color.themeName"
                      @click="chooseTheme(color.themeName)"
                    />
                  </div>
                </section>
                ...
                <AppCustomizerControls
                  ...
                  @update:text-field-variant="emit('update:textFieldVariant', $event)"
                  @update:text-field-height="emit('update:textFieldHeight', $event)"
                ...
                <section>
                  ...
                  <div class="app-customizer-panel__surfaces">
                    <button
                      ...
                      :key="style.value"
                      :class="['surface-preview', `surface-preview--${style.value}`, { active: surfaceStyle === style.value }]"
                      @click="emit('update:surfaceStyle', style.value)"
                    >
                      <span class="surface-preview__sample" aria-hidden="true"><i /><b /><em /></span>
                      <span class="surface-preview__copy"
                        ><strong>{{ style.title }}</strong
                        ><small>{{ style.description }}</small></span
                      >
                    </button>
                  </div>
...
<style scoped>
.app-customizer-panel :deep(.v-navigation-drawer__content) {
  display: flex;
  flex-direction: column;
  overflow: hidden;
  background: rgb(var(--v-theme-surface));
}
.app-customizer-panel__scroll {
  flex: 1 1 auto;
  min-height: 0;
  overflow: auto;
}
.app-customizer-panel__header {
  padding: 18px 20px 14px;
  display: flex;
  align-items: center;
  justify-content: space-between;
  border-bottom: 1px solid rgba(var(--v-theme-borderLight), 0.45);
}
.app-customizer-panel__content {
  display: grid;
  gap: 24px;
}
.app-customizer-panel__content section {
  display: grid;
  gap: 12px;
}
.app-customizer-panel__direction { display: flex !important; align-items: center; justify-content: space-between; gap: 12px; }
.app-customizer-panel__direction h6 { margin: 0; }
.app-customizer-panel__direction-options { display: flex; flex: 0 0 auto; gap: 4px; }
.app-customizer-panel__direction-options :deep(.v-btn) { height: 44px; width: 44px; min-width: 44px; padding: 0; border-radius: 6px; }
.direction-preview { display: block; width: 30px; height: 20px; position: relative; border: 2px solid currentColor; border-radius: 3px; opacity: .9; }
.direction-preview::before { content: ''; position: absolute; top: 3px; bottom: 3px; width: 7px; background: currentColor; opacity: .75; }
.direction-preview::after { content: ''; position: absolute; top: 4px; bottom: 4px; width: 12px; border-block: 1px solid currentColor; opacity: .55; }
.direction-preview--rtl::before { right: 3px; }
.direction-preview--rtl::after { left: 4px; }
.direction-preview--ltr::before { left: 3px; }
.direction-preview--ltr::after { right: 4px; }
.app-customizer-panel__card {
  border-radius: 0;
}
.app-customizer-panel__content h6 {
  margin: 0;
}
.app-customizer-panel__hint {
  margin: 0;
...
.sidebar-preview.closed::before {
  width: 6px;
  height: 23px;
}
.sidebar-preview.open::before {
  width: 36px;
  height: 6px;
}
.flex-1 {
...
:deep(.menu-orientation-option.active) {
  border-width: 3px !important;
  background: rgba(var(--v-theme-primary), 0.1) !important;
  box-shadow: 0 0 0 1px rgba(var(--v-theme-primary), 0.1);
}
.font-option :deep(.v-selection-control__wrapper) {
  padding: 10px 12px;
  border-radius: 8px;
  transition: background-color 0.2s ease;
}
.font-option:hover :deep(.v-selection-control__wrapper) {
  background-color: rgba(var(--v-theme-primary), 0.1);
}
.app-customizer-panel__menu-orientation {
  display: grid;
  gap: 10px;
  margin-top: 22px;
  padding-top: 20px;
...
.app-customizer-panel__width-options {
  display: flex;
  flex: 0 0 auto;
  gap: 4px;
}
.app-customizer-panel__width-options :deep(.v-btn) {
  height: 44px;
  width: 44px;
  min-width: 44px;
  padding: 0;
...
.content-width-preview--compact {
  width: 18px;
}
.content-width-preview--wide {
  width: 30px;
}

.app-customizer-panel__surfaces {
  display: grid;
...
.surface-preview:hover {
  transform: translateY(-1px);
  border-color: rgba(var(--v-theme-primary), 0.5);
}
.surface-preview.active {
  border-color: rgb(var(--v-theme-primary));
  background: rgb(var(--v-theme-lightprimary));
}
.surface-preview__sample {
  ...
  border: 1px solid rgba(var(--v-theme-borderLight), 0.7) !important;
  border-radius: 8px !important;
  background: rgb(var(--v-theme-background)) !important;
  box-shadow: none !important;
}
.surface-preview__sample i {
  grid-row: 1/-1;
  border-radius: 3px;
  background: rgb(var(--v-theme-primary));
  opacity: 0.82;
}
.surface-preview__sample b {
  border-radius: 2px;
  background: rgba(var(--v-theme-primary), 0.26);
}
.surface-preview__sample em {
  border-radius: 4px;
  background: rgb(var(--v-theme-surface));
  box-shadow: 0 4px 7px rgba(0, 0, 0, 0.1);
}
.surface-preview--elevated .surface-preview__sample em {
  box-shadow: 0 7px 10px rgba(18, 30, 49, 0.2);
}
.surface-preview--rounded .surface-preview__sample,
.surface-preview--rounded .surface-preview__sample em {
  border-radius: 22px !important;
}
.surface-preview--rounded .surface-preview__sample i {
  border-radius: 16px;
}
.surface-preview--tonal .surface-preview__sample {
  background: linear-gradient(135deg, rgb(var(--v-theme-lightprimary)), rgb(var(--v-theme-lightsecondary))) !important;
}
.surface-preview--tonal .surface-preview__sample em {
  box-shadow: none;
  border: 1px solid rgba(var(--v-theme-primary), 0.16);
}
.surface-preview--premium-executive .surface-preview__sample {
  background: #142137 !important;
  border-color: #263754 !important;
}
.surface-preview--premium-executive .surface-preview__sample i {
  background: #263954;
}
.surface-preview--premium-executive .surface-preview__sample b {
  background: rgba(255, 255, 255, 0.18);
}
.surface-preview--premium-executive .surface-preview__sample em {
  background: #f7f9fc;
  box-shadow: 0 4px 8px rgba(0, 0, 0, 0.22);
}
.surface-preview__copy {
  display: grid;
  gap: 2px;
}
...
:global(.surface-premium-executive) .app-customizer-panel :deep(.v-navigation-drawer__content) {
  background: #fff;
}
:global(.surface-premium-executive[class*='v-theme--Dark']) .app-customizer-panel :deep(.v-navigation-drawer__content) {
  background: #182333;
}
:global(.surface-premium-executive[class*='v-theme--Dark']) .app-customizer-panel__header,
:global(.surface-premium-executive[class*='v-theme--Dark']) .app-customizer-panel__actions {
  background: #182333;
  border-color: #2b3b4f;
}
:global(.surface-premium-executive[class*='v-theme--Dark']) .app-customizer-panel__hint {
  color: #b5c2d2;
}
</style>
