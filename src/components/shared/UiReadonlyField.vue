...
const text = computed(() => props.value === null || props.value === undefined || props.value === '' ? props.emptyText : typeof props.value === 'boolean' ? (props.value ? 'بله' : 'خیر') : String(props.value));
async function copy() { await navigator.clipboard?.writeText(text.value); }
</script>
<template><div class="ui-readonly-field"><div class="ui-readonly-field__label">{{ label }}<small v-if="secondaryLabel">{{ secondaryLabel }}</small></div><div class="ui-readonly-field__value" :dir="direction"><slot :value="value" :text="text">{{ text }}</slot><v-btn v-if="copyable && text !== emptyText" icon="$copy" size="x-small" variant="text" :aria-label="`کپی ${label}`" @click="copy"/></div></div></template>
